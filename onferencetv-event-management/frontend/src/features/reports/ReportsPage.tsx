...
import { BarChart3, CalendarDays, Sparkles, FileText, CheckCircle2 } from 'lucide-react';
...
const COLORS = ['#4f46e5', '#8b5cf6', '#ec4899', '#f43f5e', '#f97316', '#eab308', '#22c55e', '#0ea5e9'];
...
export function ReportsPage() {
  ...
  const { data: pdfHistory = [], isLoading: pdfLoading } = useGetPdfHistoryQuery();
  ...
  const eventsByMonthData = useMemo(() => {
    ...
    return Object.entries(counts).map(([name, count]) => ({ name, Events: count }));
  }, [events]);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto pb-24">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Comprehensive overview of your event management operations and AI usage.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card className="shadow-sm border-0 bg-white dark:bg-zinc-950">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-medium text-muted-foreground">Upcoming / Completed</p>
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-muted-foreground font-medium">/ {completedEvents}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-0 bg-white dark:bg-zinc-950">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-medium text-muted-foreground">AI Content Generates</p>
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-muted-foreground ml-2">Total uses</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-0 bg-white dark:bg-zinc-950">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-medium text-muted-foreground">PDF Exports</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="h-[300px] w-full">
              {eventsByMonthData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={eventsByMonthData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey="Events" fill="#4f46e5" radius={[4, 4, 0, 0]} barSize={40} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
        ...
        <Card className="shadow-sm border-0">
          <CardHeader>
            ...
            <CardDescription>Generations performed over recent days</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px] w-full">
              {aiGenerationsData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={aiGenerationsData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                    <defs>
                ...
                <div className="h-full flex items-center justify-center text-muted-foreground">Not enough AI usage data</div>
              )}
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {speakerData.map((entry, index) => (
                <div key={entry.name} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="truncate max-w-[150px] font-medium">{entry.name}</span>
                  </div>
                  <span className="text-muted-foreground">{entry.value} events</span>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="shadow-sm border-0 lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Latest actions taken across the workspace</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              {/* Mix and sort recent events, ai, and pdfs to create a fake timeline */}
              {[
                ...events.map(e => ({ type: 'event', title: `Created event: ${e.eventName}`, time: new Date(e.createdAt) })),
                ...aiHistory.map(a => ({ type: 'ai', title: `Generated AI ${a.promptType} for ${a.eventName}`, time: new Date(a.timestamp) })),
                ...pdfHistory.map(p => ({ type: 'pdf', title: `Exported PDF for ${p.eventName}`, time: new Date(p.timestamp) }))
              ]
                ...
                .map((item, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className={`mt-0.5 p-2 rounded-full ${
                      item.type === 'event' ? 'bg-indigo-100 text-indigo-600' :
                      item.type === 'ai' ? 'bg-purple-100 text-purple-600' :
                      'bg-blue-100 text-blue-600'
                    }`}>
                      {item.type === 'event' && <CalendarDays className="h-4 w-4" />}
                ...
                {events.length === 0 && aiHistory.length === 0 && pdfHistory.length === 0 && (
