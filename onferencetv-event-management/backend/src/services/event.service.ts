...
export const generateContent = async (id: string) => {
  ...
  if (env.OPENAI_API_KEY && env.AI_PROVIDER === 'openai') {
    // Basic fetch to OpenAI without an extra library for simplicity,
    // or we just use placeholder if no fetch is preferred.
    // Given requirements say to use a deterministic local placeholder if not configured,
    // we'll implement that and you can replace it if needed.
